...
define("quill-nbb", [
  ...
  $(window).on("action:composer.loaded", (ev, data) => {
    ...
    if (cidEl.length) {
      ...
      postContainer.append(
        `<input id="cmp-cid-${data.post_uuid}" type="hidden" value="${ajaxify.data.cid}"/>`
      );
    }

    // (topic thumbnail code omitted)
  ...
  $(window).on("action:composer.check", (ev, data) => {
    ...
    const quill = components
      .get("composer")
      .filter(`[data-uuid="${data.post_uuid}"]`)
      .find(".ql-container")
      .data("quill");
  ...
  $(window).on("action:chat.sent", (evt, data) => {
    // Empty chat input
    const quill = $(
      `.chat-modal[data-roomid="${data.roomId}"] .ql-container, .expanded-chat[data-roomid="${data.roomId}"] .ql-container`
    ).data("quill");
  ...
  $(window).on("action:composer.upload", (evt, data) => {
    ...
    data.files.forEach((file) => {
      ...
      // mark complete
      const key = file.filename.replace(/^\d+_\d+_/, "");
      delete window.quill.uploads[key];
    });
  });
  ...
  $(window).on("action:composer.uploadError", (evt, data) => {
    const quill = components
      ...
      .find(".ql-container")
      .data("quill");
    const textareaEl = components
      .get("composer")
      .filter(`[data-uuid="${data.post_uuid}"]`)
      .find("textarea");
    textareaEl.val(
      !window.quill.isEmpty(quill) ? JSON.stringify(quill.getContents()) : ""
    );
    textareaEl.trigger("change");
    textareaEl.trigger("keyup");

    // clear any in-flight markers for these files
    (data.files || []).forEach((f) => {
      const key = (f && f.filename ? f.filename : "").replace(/^\d+_\d+_/, "");
      if (key) delete window.quill.uploads[key];
      const alertId = key ? generateAlertId(data.post_uuid, key) : undefined;
      if (alertId) alerts.remove(alertId);
    });

    // optional toast:
    if (data && data.error) alerts.error(data.error);
  });

  $(window).on("action:composer.uploadStart", (evt, data) => {
    data.files.forEach((file) => {
      // track so uploadUpdate can find it
      const key = file.filename.replace(/^\d+_\d+_/, "");
      window.quill.uploads[key] = true;
  ...
  $(window).on("action:chat.updateRemainingLength", (evt, data) => {
    ...
    data.parent
      ...
      .text(config.maximumChatMessageLength - length);
  });
...
$(window).on("action:chat.loaded", (evt, containerEl) => {
  require(["composer", "composer/autocomplete", "components"], (
    composer,
    autocomplete,
    components
  ) => {
    // Create div element for composer
    const targetEl = $("<div></div>").insertBefore(
      components.get("chat/input")
    );
    ...
    window.quill.configureToolbar(targetEl, data).then(({ toolbar }) => {
      ...
      ["upload:post:image", "upload:post:file"].forEach((privilege) => {
        if (app.user.privileges[privilege]) {
          const className =
            privilege === "upload:post:image" ? "picture" : "upload";
      ...
      const doAutocomplete = function () {
        ...
        return !$(`.composer-autocomplete-dropdown-${data.post_uuid}:visible`)
          .length;
      };
      [9, 13].forEach((keyCode) => {
        ...
        quill.keyboard.bindings[keyCode].unshift(
          quill.keyboard.bindings[keyCode].pop()
        );
      });
...
window.quill.configureToolbar = async (targetEl, data) => {
  ...
  data.formatting.forEach((option) => {
    ...
    toolbar.handlers[option.name] = function () {
      ...
      const selection = quill.getSelection(true);
      toolbarHandlers[option.name].apply(data.postContainer, [
        textareaEl.get(0),
        selection.index,
